                    ...
                    <?php if ($page->vita()->isNotEmpty()) : ?>
                        ...
                        <?php foreach ($page->vita()->toStructure() as $entry) : ?>
                            ...
                            <div class="flex flex-row flex-wrap md:flex-no-wrap items-baseline pb-1">
                                ...
                                <?php if ($entry->itemtype() == 'heading') : ?>

                                <div class="w-full">
                                    <h3 class="text-base xl:text-lg text-wilberg-text font-bold"><?= $entry->heading(); ?></h3>
                                </div>

                                <?php else : ?>

                                <div class="w-full md:w-40 flex-none text-base text-white pr-2">
                                    <?= $entry->date(); ?>
                                </div>

                                <div class="w-full md:w-auto flex-none md:flex-auto text-base xl:text-lg text-wilberg-text">
                                    <?= $entry->contents()->kt(); ?>
                                </div>
